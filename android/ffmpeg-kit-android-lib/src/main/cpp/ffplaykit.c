...
const char program_name_ffplaykit[] = "ffplay";
 ...
 */
void ffplaykit_log_callback_function(void *ptr, int level, const char* format, va_list vargs) {
    AVBPrint fullLine;
    AVBPrint part[4];
    int print_prefix = 1;
...
static const OptionDef options[] = {
    ...
    {"ffplay_find_stream_info", OPT_BOOL | OPT_INPUT | OPT_EXPERT, {&ffplay_find_stream_info}, "read and decode the streams to fill missing information with heuristics"},
...
void jstringToChar(JNIEnv *env, jstring jstr, char **out)
{
...
{
    int returnCode = 0;
    LOGI("nativeFFplayMain!");

    int argc = 0;
    char **argv = NULL;

    char *char_argu;
    jstringToChar(env, arguments, &char_argu);

    argv = (char**)malloc(strlen(char_argu) * sizeof(char *));
    char *token = strtok(char_argu, " ");
    while (token != NULL)
    {
